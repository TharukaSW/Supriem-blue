...
import { TxType, InvoiceType, DocStatus } from '@prisma/client';
...
export class ReportsService {
    ...
    async getSalesReport(fromDate?: string, toDate?: string) {
        ...
        const totalPaid = invoices.filter(i => i.status === DocStatus.PAID).reduce((sum, i) => sum + Number(i.total), 0);
    ...
    async getPurchasesReport(fromDate?: string, toDate?: string) {
        const dateFilter = this.getDateFilter(fromDate, toDate);
    ...
    async getAttendanceReport(fromDate?: string, toDate?: string) {
        ...
        const byEmployee: Record<string, { totalDays: number; totalHours: number; totalOtHours: number }> = {};
    ...
    async getDashboardSummary() {
        ...
        const [
            ...
            this.prisma.invoice.aggregate({
                where: { invoiceType: InvoiceType.SALES, invoiceDate: { gte: startOfMonth, lte: endOfMonth } },
                _sum: { total: true },
                _count: true,
            }),
            this.prisma.invoice.aggregate({
                where: { invoiceType: InvoiceType.PURCHASE, invoiceDate: { gte: startOfMonth, lte: endOfMonth } },
                _sum: { total: true },
                _count: true,
            }),
            ...
            this.prisma.salesOrder.count({ where: { status: DocStatus.DRAFT } }),
    ...
    private getDateFilter(fromDate?: string, toDate?: string, field: string = 'date') {
